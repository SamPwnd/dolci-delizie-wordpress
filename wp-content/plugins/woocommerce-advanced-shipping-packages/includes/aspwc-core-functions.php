...
function aspwc_split_cart_shipping_packages( $packages ) {
	...
	$default_key     = array_key_first( $origin_packages );
	...
	if ( ! empty( $default_package['contents'] ) ) {
		$packages[ $default_key ] = $default_package;
	}
...
function aspwc_shipping_package_name( $name, $i, $package ) {
	...
	$asp_id = $package['asp_id'] ?? ( ( is_numeric( $i ) && 'shipping_package' == get_post_type( $i ) ) ? $i : null );
	if ( $asp_id ) {
...
function aspwc_checkout_store_package_name( $item, $package_key, $package, $order ) {
	$asp_id = $package['asp_id'] ?? ( is_numeric( $package_key ) && 'shipping_package' == get_post_type( $package_key ) ) ? $package_key : null;
	...
	// Default package name
	if ( $package_key === 0 && $title = get_option( 'advanced_shipping_packages_default_package_name', '' ) ) { // Default package
		$name = $title;
	}
...
function aspwc_get_shipping_to_display( $item, $order, $tax_display = '' ) {
	...
	if ( 0 < abs( (float) $total ) ) {
		...
		if ( 'excl' === $tax_display ) { // Show shipping excluding tax.
			...
			$shipping = wc_price( $total + $item->get_total_tax(), array( 'currency' => $order->get_currency() ) );
