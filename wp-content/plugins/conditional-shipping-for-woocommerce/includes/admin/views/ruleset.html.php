...
<h2 class="woo-conditional-shipping-heading">
	<a href="<?php echo admin_url( 'admin.php?page=wc-settings&tab=shipping&section=woo_conditional_shipping' ); ?>"><?php esc_html_e( 'Conditions', 'woo-conditional-shipping' ); ?></a>
	 &gt; 
	...
	<tbody>
		<tr valign="top" class="">
			...
			<td class="forminp">
				...
				<label for="ruleset_enabled"><?php esc_html_e( 'Enable ruleset', 'woo-conditional-shipping' ); ?></label>
			</td>
		</tr>
		<tr valign="top" class="">
			<th scope="row" class="titledesc">
				<label>
					<?php esc_html_e( 'Title', 'woo-conditional-shipping' ); ?>
			...
			<td class="">
				...
				<?php if ( ! class_exists( 'Woo_Conditional_Shipping_Pro' ) ) { ?>
					<p class="description conditions-desc wcs-pro-promo">
						<?php printf( __( 'More conditions and actions available in the Pro version. <a href="%s" target="_blank">Check out all the differences &raquo;</a>', 'woo-conditional-shipping' ), 'https://wptrio.com/guide/woocommerce-conditional-shipping-free-vs-pro/' ); ?>
					</p>
				<?php } ?>
			</td>
		</tr>
			...
			<td class="">
				...
				>
					...
					<tfoot>
						<tr>
							<td colspan="4" class="forminp">
								<button type="button" class="button" id="wcs-add-action"><?php esc_html_e( 'Add Action', 'woo-conditional-shipping' ); ?></button>
							</td>
						</tr>
					</tfoot>
				</table>
		...
		<?php if ( ! class_exists( 'Woo_Conditional_Shipping_Pro' ) ) { ?>
			<tr valign="top" class="">
				<th scope="row" class="titledesc">
					<label>
						<?php esc_html_e( 'Pro features', 'woo-conditional-shipping' ); ?>
					</label>
				</th>
				<td class="forminp">
					<input type="checkbox" name="wcs_pro_features" id="wcs_pro_features" value="1" <?php checked( get_option( 'wcs_pro_features', true ) ); ?> />
					<label for="wcs_pro_features"><?php echo sprintf( __( 'Display features available in <a href="%s" target="_blank">Pro</a>', 'woo-conditional-shipping' ), 'https://wptrio.com/products/conditional-shipping/' ); ?></label>
				</td>
			</tr>
		<?php } ?>
	</tbody>
	...
	<tr valign="top" class="condition_row">
		<td class="wcs-condition">
			<select name="wcs_conditions[{{data.index}}][type]" class="wcs_condition_type_select">
				<option value=""><?php echo wcs_esc_html( __( '- Select condition - ', 'woo-conditional-shipping' ) ); ?></option>
			...
			<div class="wcs-operator-inputs">
				<div class="value_input wcs_product_measurement_mode_input">
					<select name="wcs_conditions[{{data.index}}][product_measurement_mode]" class="">
						<option value="highest" <# if ( data.product_measurement_mode && data.product_measurement_mode == 'highest' ) { #>selected<# } #>><?php esc_html_e( 'highest', 'woo-conditional-shipping' ); ?></option>
						<option value="lowest" <# if ( data.product_measurement_mode && data.product_measurement_mode == 'lowest' ) { #>selected<# } #>><?php esc_html_e( 'lowest', 'woo-conditional-shipping' ); ?></option>
					</select>
				</div>
		...
		<td class="wcs-values">
			...
			<div class="value_input wcs_subtotal_value_input">
				...
				<label for="wcs-subtotal-includes-coupons-{{data.index}}"><?php esc_html_e( 'Subtotal includes coupons', 'woo-conditional-shipping' ); ?></label>
			</div>
			...
			<div class="value_input wcs_product_value_input">
				<select class="wc-product-search" multiple="multiple" name="wcs_conditions[{{data.index}}][product_ids][]" data-placeholder="<?php esc_attr_e( 'Search for products', 'woo-conditional-shipping' ); ?>" data-action="woocommerce_json_search_products_and_variations">
			...
			<div class="value_input wcs_product_tag_value_input">
				<select class="wcs-tag-search" multiple="multiple" name="wcs_conditions[{{data.index}}][product_tags][]" data-placeholder="<?php esc_attr_e( 'Search for tags', 'woo-conditional-shipping' ); ?>">
			...
			<div class="value_input wcs_coupon_value_input">
				<select class="wcs-coupon-search" multiple="multiple" name="wcs_conditions[{{data.index}}][coupon_ids][]" data-placeholder="<?php esc_attr_e( 'Search for coupons', 'woo-conditional-shipping' ); ?>">
			...
			<div class="value_input wcs_postcode_value_input">
				...
				<div class="wcs-desc"><?php esc_html_e( 'Postcodes containing wildcards (e.g. CB23*) or fully numeric ranges (e.g. <code>90210...99000</code>) are also supported.', 'woo-conditional-shipping' ); ?></div>
			</div>
	...
	<tr valign="top" class="action_row">
		<td class="wcs-action">
			<select name="wcs_actions[{{data.index}}][type]" class="wcs_action_type_select">
				<option value=""><?php echo wcs_esc_html( __( '- Select action - ', 'woo-conditional-shipping' ) ); ?></option>
		...
		<td class="wcs-methods">
			<select name="wcs_actions[{{data.index}}][shipping_method_ids][]" multiple class="select wc-enhanced-select" data-placeholder="<?php echo esc_attr( __( '- Select shipping methods -', 'woo-conditional-shipping' ) ); ?>">
			...
			<div class="wcs-match-by-name">
				...
				<div class="wcs-desc"><?php esc_html_e( 'Match shipping methods by name. Wildcards (e.g. DHL Express*) are also supported. Enter one name per line.', 'woo-conditional-shipping' ); ?></div>
			</div>
